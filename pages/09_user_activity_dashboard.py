...
if uploaded_file is not None:
    try:
        ...
        if missing_columns:
            ...
            st.info(
                "Required columns: country, division, fullName, fromDate, toDate, logins"
            )
            st.stop()
        ...
        df["week_period"] = (
            df["fromDate"].dt.strftime("%Y-%m-%d")
            + " to "
            + df["toDate"].dt.strftime("%Y-%m-%d")
        )
        ...
        agg_dict = {"logins": "sum", "week_period": "count", "createEvents": "sum"}

        user_totals = (
            df.groupby(["fullName", "country", "division"]).agg(agg_dict).reset_index()
        )
        user_totals.columns = [
            "fullName",
            "country",
        ...
        active_users = active_users.sort_values(
            ["country", "total_logins"], ascending=[True, False]
        )

        # Filter users with createEvents
        active_users_events = user_totals[user_totals["total_createEvents"] > 0].copy()
        active_users_events = active_users_events.sort_values(
            ["country", "total_createEvents"], ascending=[True, False]
        )
        ...
        weekly_summary = (
            df.groupby(["fromDate", "week_period"]).agg(weekly_agg_dict).reset_index()
        )
        ...
        weekly_by_country = (
            df.groupby(["country", "fromDate", "week_period"])
            .agg(weekly_agg_dict)
            .reset_index()
        )
        weekly_by_country.columns = [
            "country",
            "fromDate",
            "week_period",
            "total_logins",
        ...
        for i, country in enumerate(countries_for_chart):
            ...
            ax1.plot(
                country_data["fromDate"],
                country_data["total_logins"],
                marker="o",
                linewidth=2.5,
                markersize=6,
                color=color,
        ...
        ax1.set_title(
            "Total Logins per Week by Country", fontsize=12, fontweight="bold"
        )
        ...
        ax2.set_title(
            "Active Users per Week by Country", fontsize=12, fontweight="bold"
        )
        ...
        weekly_display.columns = [
            column_rename.get(col, col) for col in weekly_display.columns
        ]
        ...
        countries_in_data = [
            country
            for country in total_users_by_country.keys()
            if country in mau_by_country
        ]
        ...
        if countries_in_data:
            ...
            for i, country in enumerate(countries_in_data):
                ...
                # Ensure inactive count is not negative (in case actual users > hardcoded total)
                ...
                axes[i].set_title(
                    f"{country}\nMAU: {active_users_count}/{total_users}",
                    fontsize=12,
                    fontweight="bold",
                )
            ...
            for i, country in enumerate(countries_in_data):
                ...
                with cols[i]:
                    st.metric(
                        f"{country} MAU Rate",
                        f"{(active_users_count/total_users*100):.1f}%",
                    )
        else:
            st.info(
                "No data available for the specified countries (Singapore, Malaysia, Vietnam, Philippines)"
            )
        ...
        top_n = st.slider(
            "Select top N users per country", min_value=3, max_value=10, value=5
        )

        # Get top N users per country (by total logins)
        top_users_by_country = (
            active_users.groupby("country").head(top_n).reset_index(drop=True)
        )
        ...
        for i, country in enumerate(countries):
            with tabs[i]:
                country_data = active_users[active_users["country"] == country].head(
                    top_n
                )
                if not country_data.empty:
                    st.write(
                        f"**Top {min(top_n, len(country_data))} Users in {country}:**"
                    )
                    display_df = country_data[
                        ["fullName", "division", "total_logins", "weeks_active"]
                    ].copy()
                    display_df.columns = [
                        "Full Name",
                        "Division",
        ...
        top_users_overall = active_users.sort_values(
            "total_logins", ascending=False
        ).head(top_n * 4)
        user_labels = [
            f"{name}\n({country})"
            for name, country in zip(
                top_users_overall["fullName"], top_users_overall["country"]
            )
        ]

        # Use consistent color scheme
        bars2 = ax.bar(
            range(len(top_users_overall)),
            top_users_overall["total_logins"],
        ...
        ax.set_title(
            f"Top {len(top_users_overall)} Users Overall",
            fontsize=12,
            fontweight="bold",
        )
        ...
        for i, value in enumerate(top_users_overall["total_logins"]):
            ax.text(
                i, value + 0.1, str(value), ha="center", va="bottom", fontweight="bold"
            )
        ...
        country_totals = (
            active_users.groupby("country")["total_logins"]
            .sum()
            .sort_values(ascending=True)
        )
        colors = [
            COLORS_QUALITATIVE[i % len(COLORS_QUALITATIVE)]
        ...
        most_active_country = (
            active_users.groupby("country")["total_logins"].sum().idxmax()
        )
        ...
        st.write(
            f"• **Average Logins per Active User**: {active_users['total_logins'].mean():.1f}"
        )
        st.write(
            f"• **Most Active User**: {most_active_user['fullName']} from {most_active_user['country']} with {most_active_user['total_logins']} total logins across {most_active_user['weeks_active']} weeks"
        )
        st.write(
            f"• **Average Weeks Active per User**: {user_totals['weeks_active'].mean():.1f}"
        )
        st.write(
            f"• **Most Active Week**: {most_active_week['week_period']} with {most_active_week['total_logins']} total logins"
        )
        st.write(
            f"• **Most Active Country**: {most_active_country} with {active_users[active_users['country'] == most_active_country]['total_logins'].sum()} total logins"
        )
        ...
        with col3:
            avg_events = (
                active_users_events["total_createEvents"].mean()
        ...
        if len(active_users_events) > 0:
            ...
            for i, country in enumerate(countries):
                with tabs[i]:
                    country_data = active_users_events[
                        active_users_events["country"] == country
                    ]
                    if not country_data.empty:
                        st.write(
                            f"**{len(country_data)} Users** with Create Events in {country}:"
                        )
                        display_df = country_data[
                            [
                                "fullName",
            ...
            top_users_events_overall = active_users_events.sort_values(
                "total_createEvents", ascending=False
            ).head(top_n * 3)
            user_labels_events = [
                f"{name}\n({country})"
                for name, country in zip(
                    top_users_events_overall["fullName"],
                    top_users_events_overall["country"],
                )
            ]

            # Use consistent color scheme (green for events)
            bars_events = ax.bar(
                range(len(top_users_events_overall)),
                top_users_events_overall["total_createEvents"],
                color=COLORS_SEQUENTIAL_GREEN(
                    np.linspace(0.4, 0.9, len(top_users_events_overall))
                ),
                edgecolor="white",
                linewidth=0.5,
            )
            ...
            ax.set_title(
                f"Top {len(top_users_events_overall)} Users Overall by Create Events",
                fontsize=12,
                fontweight="bold",
            )
            ...
            country_totals_events = (
                active_users_events.groupby("country")["total_createEvents"]
                .sum()
                .sort_values(ascending=True)
            )
            colors_events = [
                COLORS_QUALITATIVE[i % len(COLORS_QUALITATIVE)]
                for i in range(len(country_totals_events))
            ]

            bars_events_country = ax.barh(
                country_totals_events.index,
                country_totals_events.values,
                color=colors_events,
                edgecolor="white",
                linewidth=1,
            )
            ...
            ax.set_title(
                "Total Create Events by Country", fontsize=12, fontweight="bold"
            )
            ax.grid(axis="x", alpha=0.3, linestyle="--")
            ax.set_facecolor(COLOR_BACKGROUND)
            ...
            weekly_events_summary = (
                df.groupby(["fromDate", "week_period"])
                .agg({"createEvents": "sum", "fullName": "nunique"})
                .reset_index()
            )
            weekly_events_summary.columns = [
                "fromDate",
                "week_period",
                "total_createEvents",
                "active_users",
            ...
            weekly_events_summary = weekly_events_summary[
                weekly_events_summary["total_createEvents"] > 0
            ]
            ...
            if len(weekly_events_summary) > 0:
                ...
                ax.plot(
                    weekly_events_summary["fromDate"],
                    weekly_events_summary["total_createEvents"],
                    marker="o",
                    linewidth=2.5,
                    markersize=6,
                    color=COLOR_SUCCESS,
                ...
                ax.set_title(
                    "Total Create Events per Week", fontsize=12, fontweight="bold"
                )
            ...
            st.subheader("💡 Key Insights")
            most_active_country_events = (
                active_users_events.groupby("country")["total_createEvents"]
                .sum()
                .idxmax()
            )
            most_active_user_events = active_users_events.loc[
                active_users_events["total_createEvents"].idxmax()
            ]

            st.write(
                f"• **Users Creating Events**: {len(active_users_events)} out of {len(user_totals)} users ({len(active_users_events)/len(user_totals)*100:.1f}%)"
            )
            st.write(
                f"• **Average Events per Active User**: {active_users_events['total_createEvents'].mean():.1f}"
            )
            st.write(
                f"• **Most Active User**: {most_active_user_events['fullName']} from {most_active_user_events['country']} with {int(most_active_user_events['total_createEvents'])} total events"
            )
            st.write(
                f"• **Most Active Country**: {most_active_country_events} with {int(active_users_events[active_users_events['country'] == most_active_country_events]['total_createEvents'].sum())} total events created"
            )
        else:
        ...
        view_columns = [
            ...
            if "view" in col.lower() or "create" in col.lower()
        ]
        activity_totals = {}
        ...
        if activity_totals:
            ...
            bars = ax.bar(
                activities, values, color=colors, edgecolor="white", linewidth=0.5
            )
            ax.set_title(
                "Total Activity Breakdown (All Weeks)", fontsize=12, fontweight="bold"
            )
            ...
            for col in view_columns:
                clean_name = (
                    col.replace("Counts", "").replace("view", "").replace("create", "")
                )
            ...
            fig_country_activity, ax = plt.subplots(
                figsize=(14, max(8, len(countries) * 0.5))
            )
            ...
            ax.set_title(
                f"Top {top_n_activities} Activities by Country (Stacked)",
                fontsize=12,
                fontweight="bold",
            )
            ax.legend(
            ...
            top_5_df.plot(
                ...
                color=[
                    COLORS_QUALITATIVE[i % len(COLORS_QUALITATIVE)]
                    for i in range(len(top_5_activities))
                ],
                width=0.7,
                edgecolor="white",
                linewidth=0.5,
            ...
            ax.set_ylabel("Country", fontsize=10)
            ax.set_title(
                "Top 5 Activities by Country (Grouped Comparison)",
                fontsize=12,
                fontweight="bold",
            )
            ...
            for country in countries:
                ...
                country_row["Total Activities"] = sum(
                    [v for k, v in country_row.items() if k != "Country"]
                )
        ...
        st.info(
            "Please make sure your CSV file is properly formatted and contains the required columns."
        )
...
# =============================================================================
    ...
    st.write(
        "Your CSV file should contain weekly user activity data with the following columns:"
    )
    ...
    st.write(
        "- **Various view counts**: Additional activity metrics (viewHomeCounts, createEvents, etc.)"
    )
