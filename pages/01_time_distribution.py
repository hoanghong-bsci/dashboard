...
def plot_pie_chart_multi_col(summary_df, charts_per_row=3):
    ...
    """
    filtered_users = [
        user for user in summary_df.index if (summary_df.loc[user] > 0).any()
    ]
    ...
    for i in range(0, len(filtered_users), charts_per_row):
        ...
        for idx, user in enumerate(row_users):
            with cols[idx]:
                ...
                if user_data.empty or user_data.sum() == 0:
                    st.write(f"{user}: No data")
                    continue
...
st.write(
    "Then upload the generated CSV file: `timesheet_entries.csv` below to view the time distribution"
)
...
if distribution_csv is not None:
    try:
        ...
        df_summary = df_summary.apply(pd.to_numeric, errors="coerce").fillna(0)
        df_summary = df_summary.clip(lower=0)  # Replace negative values with 0
