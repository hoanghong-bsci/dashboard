...
def extract_user_row_mappings(df):
    ...
    summary_categories = [
        "Absences [h]",
        "Networking and administration, personal development",
        "Operational hours",
    ]
...
def extract_date_col_mappings(df):
    ...
    columns = (
        df.iloc[2, 15:].dropna().astype(str).tolist()
    )  # Start at Col P (index 15) and go to Col AT (index 45)
    valid_days = {}
...
def read_timesheet_entries_by_users(df, user_row_mappings, date_col_mappings):
    ...
    timesheet_data = pd.DataFrame(
        index=user_row_mappings.keys(), columns=date_col_mappings.keys()
    )
...
def summarise_time_distribution(df, category_row_indices, date_col_mappings):
    ...
    summary_data = {
        user: {
            "Absences [h]": 0,
            "Networking and administration, personal development": 0,
            "Operational hours": 0,
        }
        for user in category_row_indices
    }
    ...
    for user, categories in category_row_indices.items():
        for category, row_idx in categories.items():
            ...
            cell_values = df.iloc[
                row_idx + 1, [col_idx for _, col_idx in date_col_mappings.items()]
            ]
            total_hours = cell_values[
                cell_values.apply(lambda x: isinstance(x, (int, float)) and pd.notna(x))
            ].sum()
...
def main():
    ...
    timesheet_entries = read_timesheet_entries_by_users(
        df, user_row_mappings, date_col_mappings
    )
    print(timesheet_entries)
    # Load workbook with data_only=True to get cell values (not formulas)
    worksheet = load_workbook(filename=file_path, data_only=True)[
        "Sheet2"
    ]  # Replace with your sheet name
    dfs = pd.DataFrame(list(worksheet.iter_rows(values_only=True)))
    summary_data = summarise_time_distribution(
        dfs, category_row_indices, date_col_mappings
    )
    print(summary_data)
